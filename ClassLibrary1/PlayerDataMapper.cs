...
namespace Engine
{
    ...
    {
        private static readonly string _connectionString = "Data Source=LAPTOP-0468J0U6\\SQLEXPRESS;Initial Catalog=SuperAdventure;Integrated Security=True";
        ...
        public static Player CreateFromDatabase()
        {
            try
            {
                ...
                using(SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    using(SqlCommand saveGameCommand = connection.CreateCommand())
                    {
                        saveGameCommand.CommandType = CommandType.Text;
                        // This SQL statement reads teh first rows in the SavedGame table. For this program, we should only ever have one row,
                        // but this will sensure we only get one record in our SQL query results.
                        saveGameCommand.CommandText = "SELECT TOP 1 * FROM SavedGame";

                        // Use ExecuteReader when you expect the query to return a row, or rows
                        SqlDataReader reader = saveGameCommand.ExecuteReader();
                    ...
                    using (SqlCommand questCommand = connection.CreateCommand())
                    {
                        ...
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                ...
                                // Build the PlayerQuest item, from this row
        ...
        public static void SaveToDatabase(Player player)
        {
            try
            {
                using(SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    // The Quest and Inventory tables might have more, or less, rows in the database than what the player has
                    // in their properties. So, when we save the player's game, we will delete all the old rows and add in
                    // all new rows. This is easier than trying to add/delete/update each individual rows
